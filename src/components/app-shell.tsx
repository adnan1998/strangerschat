...
import { useEffect, useState } from 'react';
import { useSelector, useDispatch } from 'react-redux';
import { type RootState, type AppDispatch, store } from '@/lib/redux/store';
import { login, logout, logoutAndLeave } from '@/lib/redux/slices/sessionSlice';
import Login from '@/components/auth/login-form';
import { ChatLayout } from '@/components/chat/chat-layout';
...
import { errorEmitter } from '@/firebase/error-emitter';
import { FirestorePermissionError } from '@/firebase/errors';
import { ChatSkeletonLoader } from '@/components/chat/chat-skeleton-loader';
...
export function AppShell() {
  ...
  // Effect to synchronize Firebase Auth state with Redux session state
  useEffect(() => {
    const auth = getAuth();
    if (isUserLoading || !firestore) return;
    ...
    const handleUserSession = async () => {
      if (firebaseUser) {
        // A user is authenticated with Firebase
        
        // Ensure the global chat room document exists, now that we know we have a user
        const globalChatRoomRef = doc(firestore, 'chat_rooms', 'global');
        getDoc(globalChatRoomRef).then(docSnap => {
          if (!docSnap.exists()) {
            setDocumentNonBlocking(globalChatRoomRef, {
        ...
        // Scenario 1: A new user has just completed the login form.
        const pendingUserDetailsStr = localStorage.getItem('pending-user-details');
        if (pendingUserDetailsStr) {
          const pendingUserDetails = JSON.parse(pendingUserDetailsStr);
          const newUser: User = {
          ...
          // Create the user documents and log them in
          setDocumentNonBlocking(userDocRef, newUser, {});
          setDocumentNonBlocking(onlineUserDocRef, newUser, {});
          
          dispatch(login(newUser));
          localStorage.removeItem('pending-user-details');
          return; // Session is now active
        }

        // Scenario 2: A returning user is already logged into Firebase, but Redux state is not yet initialized.
        if (!sessionUser) {
          setIsUserDataLoading(true);
          try {
            const userDocRef = doc(firestore, 'users', firebaseUser.uid);
            ...
            if (docSnap.exists()) {
              ...
              // Mark user as online in Firestore and update Redux state
              setDocumentNonBlocking(onlineUserDocRef, { ...userData, isOnline: true }, { merge: true });
              dispatch(login(userData));
            } else {
              // This is a critical error state: user is in Firebase Auth but not in Firestore.
              // The safest action is to sign them out to force a clean login.
              await auth.signOut();
              dispatch(logout());
            }
          } catch (error) {
             ...
             errorEmitter.emit('permission-error', contextualError);
             // If we can't fetch their data, we must log them out.
             await auth.signOut();
             dispatch(logout());
          } finally {
            setIsUserDataLoading(false);
        ...
        // No Firebase user. Ensure the Redux state is logged out.
        if (isLoggedIn) {
          dispatch(logout());
        }
      }
  ...
  // Effect to handle user leaving the page (e.g., closing tab)
  useEffect(() => {
    const handleBeforeUnload = () => {
      // This only triggers if the user is fully logged in.
